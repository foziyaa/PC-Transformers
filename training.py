...
def load_local_tokenizer(path):
    ...
    return GPT2TokenizerFast.from_pretrained(os.path.dirname(path), 
                                             tokenizer_file=os.path.basename(path))
        ...
        for module in model.modules():
            if isinstance(module, PCLayer) and hasattr(module, "get_energy"):
                ...
                if energy is None or (isinstance(energy, float) and math.isnan(energy)):
                    continue
                if getattr(module, 'layer_type', None) == 'linear_output':
...
def main():
    ...
    if rank == 0:
        file_h = logging.FileHandler(os.path.join(LOG_DIR, "training.log"))
